...
从 develop 分支创建一个新的 release 分支的关键时刻是，develop （几乎）反应了新发布特性期望的状态。至少在哪个时间点，所有将要构建版本的 feature 分支合并到 develop 中。所有针对未来版本的 feature 分支可能不需要等到 release 分支建立之后。

release 分支的开始，会获得一个新的版本号。在那之前，develop 分支反应了 “下一个版本”的变化，但是不清楚“下一个版本”最终会变成0.3还是1.0，直到 release 分支启动。这个决定是在 release 分支开始时做出的，并由项目的版本规则得出版本号。

### 创建 release 分支
release 分支基于 develop 分支创建。例如目前生产发布的版本是 1.1.5 ，将有一个大发布版本到来。develop 分支的状态已为“下一个版本”做好准备，并且决定这个版本将会变成 1.2 （不是 1.1.6 或 2.0）。因此，给 release 分支一个反应新版本号的名称：
```git
...
```
在这里，bump-version.sh是一个虚构的shell脚本，它更改工作副本中的一些文件以反映新版本。

这个新分支可能会在哪里存在一段时间，知道发布版明确推出。在这段时间内，bug 修复可以应用在这个分支（而不是 develop 分支）。严禁在这个分支添加大型新功能。它们必须合并到 develop ，因此要等待下一个大版本发布。

### 完成 release 分支
当release 分支的状态准备成为真正的发布版本时，需要执行一些操作。首先，release 分支合并到 master 中。下一步，必须在 master 上标记commit，以便将来方便地引用此历史版本。最后，对 release 分支所做的更改需要合并回 develop 中，以便将来的版本也包含这些bug修复。
...
为了保留在 release 分支上所做的更改，需要将这些更改合并回 develop 。
```git
$ git checkout develop
Switched to branch 'develop'
$ git merge --no-ff release-1.2
...
(Summary of changes)
```
这个过程中可能会产生冲突，如果有，就修复并提交，最后 release 可以删除，因为已经不再需要。
```git
