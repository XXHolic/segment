...
最近接触 flex 布局的时候，碰到一些问题，于是借着这个机会，对 flex 布局相关的知识点进行整理。
...
4. flex: <positive-number>：相当于 flex:<positive-number> 1 0。flex 项的大小根据 width/height 属性。盒子不会变得有弹性，跟取值 initial 有点类似，但不允许 flex 项收缩，即使在溢出的情况下。
