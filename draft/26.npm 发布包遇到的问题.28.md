...
### 发布成功了，但在 npm 上找不到包
在一次发布中，看到发布成功的提示，但到自己的 npm 账户发现没有相关的包。

反复尝试了几次，发现原来是因为自己本地 `registry` 指向了内网的一个地址，并没有指向 npm 官方的地址 `https://registry.npmjs.org/` 。所以发包的时候，先确定一下发布的指向：
```
npm config get registry
```
如果发现指向不对，后有两种修改的方式：
方式 1：使用 `npm config set registry` 改成对应的指向，但由于这个是全局修改，之后如果需要发布本地的又要修改回来。
