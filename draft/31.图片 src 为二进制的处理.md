...
2. 使用 `window.btoa()` 方法。
3. 使用 [FileReader][[url-mdn-file-reader]] 对象 `readAsDataURL()` 方法。
...
- `URL.createObjectURL()`：创建一个 `DOMString`，其中包含一个表示参数中给出的对象的 `URL`。这个 `URL` 的生命周期和创建它的窗口中的 document 绑定。这个新的URL 对象表示指定的 [File][url-mdn-file] 对象或 [Blob][url-mdn-blob] 对象。
