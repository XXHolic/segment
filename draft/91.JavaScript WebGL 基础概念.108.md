...
OpenGL 自身就是一个巨大的状态机：一系列的变量描述 OpenGL 此刻应当如何运行。在 OpenGL 相关程序中可以看到很多全局的变量，其中一些变量在某个时间点是输出，在另外一个时间点是输入。
...
**图元装配**(Primitive Assembly)将顶点着色器输出的所有顶点作为输入，所有的点会装配成指定图元的形状。
